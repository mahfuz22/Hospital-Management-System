	...
	<div class="container">
		<div>
			...
			<div id="London" class="tabcontent">
			  ...
			  <div class="adddep">
			  	<form id="form1" action="../stuff/updateMedicine.jsp" method="post">
			  	
			  		<%
			  					int id=Integer.parseInt(request.getParameter("id"));
								List<Medicine> baList=MedicineDao.getRecordById(id);
								int count=0;
								String medicineName="";
								String categoryName="";
								String description="";
								...
								int status=0;
									for(Medicine ba:baList){
											count++;
											medicineName=ba.getMedicineName();
											categoryName=ba.getCategory();
											description=ba.getDescription();
											price=ba.getPrice();
											companyName=ba.getCompanyName();
											status=(int)ba.getStatus();
									}		
					%>
			  	
			  		<div>
			  			<input type="text" name="name" value="Name: <%=medicineName %>" placeholder="Medicine Name" disabled/>
			  			<input type="hidden" name="id" value="<%=id %>"/>
			  		</div>
			  		<div>
			  			<input type="text" name="category" value="Category: <%=categoryName %>" placeholder="Medicine Name" disabled/>
			  		</div>
			  		<div>
			  			<input type="text" name="description" value="Description: <%=description %>" placeholder="Description" disabled/>
			  		</div>
			  		<div>
			  			<input type="text" name="price" value="<%=price %>" placeholder="Price"/>
			  		</div>
			  		<div>
			  			<input type="text" name="company" value="Company Name<%=companyName %>" placeholder="Manufacturing Company" disabled/>
			  		</div>
			  		<div>
